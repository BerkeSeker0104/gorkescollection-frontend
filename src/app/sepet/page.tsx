...
export default function CartPage() {
    ...
    // DÜZELTME: Ayarlar için ayrı bir yükleme durumu ekliyoruz.
    const [settingsLoading, setSettingsLoading] = useState(true);
    const [settings, setSettings] = useState<{ fee: number; threshold: number }>({
        fee: 50,
        threshold: 2000, // Varsayılanı daha mantıklı bir değere çektik.
    });

    useEffect(() => {
        const fetchSettings = async () => {
            setSettingsLoading(true); // Yüklemeyi başlat
            try {
                ...
                const threshold = parseFloat(
                    fetchedSettings.find((s: Setting) => s.key === "FreeShippingThreshold")?.value ||
                        "2000",
                );
                ...
                setSettingsLoading(false); // Yüklemeyi bitir (başarılı ya da başarısız)
            }
        };
        fetchSettings();
    }, []);
