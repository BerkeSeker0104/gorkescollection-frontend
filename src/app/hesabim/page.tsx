...
const AddressManager = () => {
  ...
  useEffect(() => {
   const log = (e: MouseEvent) => {
    ...
    if ((e.target as HTMLElement)?.id === 'saveAddressBtn') {
      console.log('[DEBUG] SaveAddressBtn click yakalandı. Top element:', el, 'class=', (el as HTMLElement)?.className);
    }
   };
  ...
  <>
    ...
    >
      ...
      <div className="flex gap-4 pt-2">
        {/* Form içi buton – RHF'yi doğrudan tetikler */}
        <button
          id="saveAddressBtn"
          type="button"
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            handleSubmit(async (vals) => { await onSubmit(vals); })();
          }}
    ...
    {/* GEÇİCİ KURTARMA BUTONU — Portal ile, overlay olsa bile çalışır */}
    <Portal>
      <div style={{ position: 'fixed', right: 24, bottom: 24, zIndex: 2147483647 }}>
        <button
          type="button"
          onClick={(e) => {
            e.preventDefault();
          ...
          disabled={isSubmitting}
          className="bg-[#2a2a2a] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-opacity-90 disabled:bg-zinc-400 transition-colors shadow-lg"
        >
          {isSubmitting ? "Kaydediliyor..." : "Adresi Kaydet"}
        </button>
      </div>
