...
// --- GÜNCELLENDİ: createOrder yerine initiatePaytrPayment import ediliyor ---
import { initiatePaytrPayment, getAddresses, getSettings } from "@/lib/api"; 
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import clsx from "clsx";
import toast from "react-hot-toast"; // Hata mesajları için toast ekleyelim
...
export default function CheckoutPage() {
    ...
    // --- YENİ: PayTR iFrame token'ını tutmak için state ---
    ...
    useEffect(() => {
        // ... (Bu useEffect bloğu aynı kalabilir, bir değişiklik yok)
    ...
    // --- YENİ: iFrame token'ı geldiyse, formu gizleyip iFrame'i göster ---
    if (iframeToken) {
        return (
    ...
    // iFrame token'ı henüz yoksa, normal ödeme formunu göster
    return (
        <div className="bg-gray-50 pt-40">
        {/* ... (Form ve sipariş özeti HTML'i aynı kalabilir, aşağıda tam halini veriyorum) */}
            <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-x-16 gap-y-12 px-4 py-16">
                ...
                <div className="bg-white p-8 rounded-lg shadow-sm self-start">
                    ...
                    <button type="submit" form="checkout-form" disabled={isSubmitting} className="w-full mt-6 rounded-md border border-transparent bg-gray-900 px-4 py-3 text-base font-medium text-white shadow-sm hover:bg-gray-700 disabled:bg-gray-400">
