...
export async function uploadToCloudinary(file: File): Promise<string> {
  ...
  if (!token) throw new Error("İmza alınamadı: Lütfen admin olarak giriş yapın (token yok).");

  const sigRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/upload-signature`, {
    method: "POST",
    headers: {
  ...
  if (!sigRes.ok) throw new Error(`İmza alınamadı (${sigRes.status}).`);
  ...
  if (!signature || !timestamp) throw new Error("Geçersiz imza yanıtı (signature/timestamp yok).");
  if (!cloudName || !apiKey) throw new Error("Cloudinary CloudName veya ApiKey eksik (API/ENV).");
  ...
  if (!upRes.ok) {
    ...
    throw new Error(`Cloudinary yükleme başarısız (${upRes.status})${errText ? `: ${errText}` : ""}`);
  }

  const json = await upRes.json();
  const secureUrl: string | undefined = json?.secure_url;
  if (!secureUrl) throw new Error("Cloudinary yanıtı geçersiz (secure_url yok).");
  return secureUrl;
}
